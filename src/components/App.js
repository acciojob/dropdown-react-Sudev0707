...
function App() {
  ...
  useEffect(() => {
    const stateData = states[selectedStateIndex];
    setSelectedState(stateData);
    setCities(stateData.city);
    setSelectedCityIndex(0);
    setSelectedCity(stateData.city[0]);
    setSelectedLandmarkIndex(0);
    if (stateData.city[0] && stateData.city[0].landmarks[0]) {
      setSelectedLandmark(stateData.city[0].landmarks[0]);
      setLandmarks(stateData.city[0].landmarks);
    }
  }, [selectedStateIndex]);
  ...
  useEffect(() => {
    if (cities[selectedCityIndex]) {
      const cityData = cities[selectedCityIndex];
      setSelectedCity(cityData);
      setLandmarks(cityData.landmarks);
      setSelectedLandmarkIndex(0);
      if (cityData.landmarks[0]) {
        setSelectedLandmark(cityData.landmarks[0]);
      }
    }
  ...
  useEffect(() => {
    if (landmarks[selectedLandmarkIndex]) {
      setSelectedLandmark(landmarks[selectedLandmarkIndex]);
    }
  }, [selectedLandmarkIndex, landmarks]);
  ...
  return (
    <div id="main">
      ...
      <div id="state-name" className="title">{selectedState.name}</div>
      <div id="state-description">{selectedState.description}</div>

      <div id="city-name" className="title">{selectedCity.name}</div>
      <div id="city-description" className="" >{selectedCity.description}</div>

      <div id="landmark-name" className="title">{selectedLandmark.name}</div>
